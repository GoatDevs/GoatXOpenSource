...
module.exports.run = async (bot, message, args) => {
    ...
    if(!helpArgs[0]) {
        var embed = new Discord.MessageEmbed()
            .setAuthor('GoatX Command List')
            .setDescription('*Want a bot? Dm ! Goat <3#9999 on discord!*')
            .addFields(
                { name: 'Usefull Commands', value: '```>nuke | >invite | >membercount```', inline: true},
                { name: 'Bot Status Commands', value: '```>status```', inline: true},
                { name: 'Fun commands', value: '```>meme | >hi | >hello```', inline: true},
                { name: 'Lookup Commands', value: '```>memberinfo```', inline: true},
                { name: 'Prefix', value: '```>```', inline: true},
                { name: 'Command Help', value: '``>help {command name here}`` will give you more info on the command your using!'},
            )
            .setColor('#00FFF3')
            .setTimestamp()
